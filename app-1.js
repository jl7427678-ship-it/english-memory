...
function defaultProjects(){return [
  {id:'toeic',name:'TOEIC',type:'language_exam',description:'背词、背诵、写作与机考训练',glyph:'T',color:'violet',visible:true,archived:false,order:0},
...
function defaultPlanner(){return {maxDailyReviews:100,schedules:[{id:'schedule-toeic-default',projectId:'toeic',title:'TOEIC 新词',mode:'quantity',amount:30,unit:'词',weekdays:[1,2,3,4,5,6,0],enabled:true}],taskStatus:{}}}
const defaultState={docs:[],activeDoc:null,study:{index:0,mode:'full'},logs:{},vocab:{decks:[],activeDeck:null,session:null,builtinInstalled:{},progress:{}},vocabLogs:{},todayChecklist:{},projects:defaultProjects(),planner:defaultPlanner(),examEngine:{questions:[],attempts:[],wrong:{},review:{},activeSession:null},questionEngine:{drafts:{},review:{},checks:{},planTasks:[],activePlanId:null},settings:{speechRate:.9,speechLang:'en-US',apiEndpoint:'',apiModel:'',apiKey:'',vocabBatchSize:'100'}};
...
function nav(page){const parent={study:'training',vocab:'training',exam:'training','exam-engine':'training','computer-exam':'training','atlas-reading':'training','language-skills':'training',review:'training','preqin-literature':'library',italiano:'library','private-library':'library',stats:'me',settings:'me'}[page]||page;$$('.page').forEach(x=>x.classList.toggle('active',x.id==='page-'+page));$$('.navbtn').forEach(x=>x.classList.toggle('active',x.dataset.page===parent));if(page==='today')renderToday();if(page==='me')renderProfileSummary();if(page==='review')renderReview();if(page==='stats')renderStats();if(page==='exam')renderExam();if(page==='exam-engine')renderExamEngine();if(page==='computer-exam'&&typeof renderComputerExam==='function')renderComputerExam();if(page==='atlas-reading'&&typeof renderAtlasReading==='function')renderAtlasReading();if(page==='language-skills'&&typeof renderLanguageSkills==='function')renderLanguageSkills();if(page==='vocab')renderVocabHome();if(page==='preqin-literature')renderPreqinCourse();if(page==='italiano'&&typeof renderItalianCourse==='function')renderItalianCourse();if(page==='private-library'&&typeof renderPrivateLibrary==='function')renderPrivateLibrary();if(page==='plan')renderCoursePlans();window.scrollTo({top:0,behavior:'smooth'})}
