...
function italianCourseText(text){return typeof italianClickableText==='function'?italianClickableText(text):esc(String(text||''))}
...
function renderItalianExercise(){
  ...
  bindItalianExercise(item);if(typeof bindItalianHelpers==='function')bindItalianHelpers(item,lesson);if($('#italianSpeak'))$('#italianSpeak').onclick=()=>speak(item.audioTarget,{userInitiated:true,lang:'it-IT'});
}
...
function italianExerciseBody(item){
  if(item.type==='select'){const prompt=item.mode==='listen'?'<p class="italian-listen-prompt">点击“朗读”，再选择听到的内容。</p>':`<div class="italian-prompt">${item.mode==='nativeToTarget'?esc(item.prompt):italianCourseText(item.prompt)}</div>`;return`${prompt}${italianAudioButton(item)}<div class="italian-options">${item.options.map((option,index)=>`<button data-italian-choice="${index}">${esc(option.text??option)}</button>`).join('')}</div>`}
  if(item.type==='fillBlank')return`<div class="italian-prompt">${italianCourseText(item.sentence)}</div><p class="sub">${esc(item.translation||'')}</p>${italianAudioButton(item)}<div class="italian-options">${item.options.map((option,index)=>`<button data-italian-choice="${index}">${esc(option)}</button>`).join('')}</div>`;
  if(item.type==='wordBank')return`<div class="italian-prompt">${italianCourseText(item.prompt)}</div>${italianAudioButton(item)}<div class="italian-built-answer" id="italianBuiltAnswer">点击下方词块组成答案</div><div class="italian-word-bank">${item.tokens.map((token,index)=>`<button data-italian-token="${index}">${esc(token)}</button>`).join('')}</div><div class="row"><button class="btn" id="italianClearTokens">清空</button><button class="btn primary" id="italianCheckTokens">检查</button></div>`;
  ...
  return`<div class="italian-prompt">${italianCourseText(item.prompt)}</div>${italianAudioButton(item)}<div class="italian-type-row"><input class="field" id="italianTypeAnswer" autocomplete="off" autocapitalize="sentences" placeholder="输入英文答案"><button class="btn primary" id="italianCheckType">检查</button></div>`;
}
