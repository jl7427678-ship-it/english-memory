...
function syncLibraryCourseShelves(){const visible=new Set((state.projects||[]).filter(project=>project.visible&&!project.archived).map(project=>project.id));if($('#italianoCourseShelf'))$('#italianoCourseShelf').hidden=!visible.has('italiano');if($('#preqinCourseShelf'))$('#preqinCourseShelf').hidden=!visible.has('chinese')}
document.addEventListener('click',event=>{if(event.target.closest('[data-project-action]'))setTimeout(syncLibraryCourseShelves,0)});
syncLibraryCourseShelves();
