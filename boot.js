(async()=>{
  ...
  try{
    const r=await fetch('ui.html');
    if(!r.ok) throw new Error('ui.html');
    mount.outerHTML=await r.text();
    await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='app.js';s.onload=resolve;s.onerror=reject;document.body.appendChild(s)});
  }catch(err){
