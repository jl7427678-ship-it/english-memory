(async()=>{
  ...
  try{
    const r=await fetch('ui.html?v=20260905-18',{cache:'no-store'});
    if(!r.ok) throw new Error('ui.html');
    mount.outerHTML=await r.text();
    await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='app.js?v=20260905-18';s.onload=resolve;s.onerror=reject;document.body.appendChild(s)});
  }catch(err){
