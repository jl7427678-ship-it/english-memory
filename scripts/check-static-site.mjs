...
for (const file of ['ui.html', 'app.js', 'app-1.js', 'app-2.js', 'app-3.js', 'app-4.js', 'app-5.js', 'app-6.js', 'app-7.js', 'app-8.js', 'app-9.js', 'app-10.js', 'app-11.js', 'app-12.js', 'app-13.js', 'data/preqin-literature.json', 'data/exam-practice.json', 'THIRD_PARTY_NOTICES.md']) {
  await exists(file);
}

assert(index.includes('boot.js?v=20260905-17'), 'index.html does not load the current boot.js version');
assert(index.includes('styles.css?v=20260905-17') && index.includes('theme.css?v=20260905-17'), 'stylesheet versions are inconsistent');
assert(boot.includes("ui.html?v=20260905-17") && boot.includes("app.js?v=20260905-17"), 'boot.js resource versions are inconsistent');
assert(app.includes("src+'?v=20260905-17'") && app.includes("'app-13.js'"), 'split application scripts are not on the current version');
...
assert(worker.includes("CACHE='english-memory-lab-v5-ui-20260905-17'") && worker.includes('app-13.js') && worker.includes('data/exam-practice.json'), 'Service Worker cache version was not bumped');
